...
function RandomGenerator({ menus = [], freqs = {}, addFrequency = (f) => f }) {
  ...
  return (
    <div className="menu-rand-wrap">
      <div className="menu-rand-left-container">
        <p class="menu-rand-fd-name">{selectedMenu.name}</p>
        <button onClick={() => clickSelect(selectedMenu)}>선택</button>
      </div>
      <div className="menu-rand-right-container">
